...
public:
    ...
    T* search(const K& key) const; // Searching function
    void insert(const T& el); // Insert Function
    bool remove(const K& key); // Returns true if element is found
...
private:
    ...
    T* search(BSTNode<T>* p, const K& key) const; // Searching Function
