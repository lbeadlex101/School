...
int main() 
{
    ...
    while(1) // Loop only breaks when jobs are serviced.
    {
        ...
        if(system == 0 && checkQ() == false) // Check for arrivals on wait q.
        {
            ...
            if(selectedN->qEstimatedRunTime ==  quantum - 1)
            {
                selectedN->qEstimatedRunTime = (selectedN->qEstimatedRunTime 
                                                - quantum);
                selectedN->qWaitTime = (selectedN->qTimeOfService 
                                        - selectedN->qTimeOfService);
                ...
                insertQ(selectedN->qJobName, selectedN->qArrivalTime, 
                        selectedN->qEstimatedRunTime,selectedN->qPriority, 
                        selectedN->qWaitTime, selectedN->qExitTime,
                        selectedN->qTimeOfService, selectedN->next);
            }
...
void seekRecords(int clockP)
{
    ...
    {
        if(selectedN->arrivalTime == clockP) // If arrival matches clock pulse.
        {
...
void readInput()
{
    ...
    while(fscanf(fin, "%s %d %d %d", jobName, &at, &ert, &p) != EOF)
    {
        ...
        jobs = (jobs + 1); // Increment number of jobs for tracking
        printf("%s %d %d %d \n", jobName, at, ert, p);
    }
