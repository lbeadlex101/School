...
#define QUANTUM (10) // Global definition for TIME QUANTUM
//------------------------------------------------------------------------------
// Data Structures
//------------------------------------------------------------------------------
struct Record
...
int main() 
{
    ...
    {
    	printf("rr_outFile.txt open for writng\n");
    }
...
//------------------------------------------------------------------------------
// Sort Queue Function (RR): Sorts the queue in regard to quantums expiring.  
void sortQueue(struct Queue* wQ)
{
	struct ListNode* target = NULL;
	
	target = getNextPointer(wQ->currentJob);
	...
	{
		wQ->currentJob = getNextPointer(wQ->currentJob);
	}
}
//------------------------------------------------------------------------------
// MISC Functions
				...
				struct Queue qObj)
{
	...
	while (isListEmpty(listObj) == false)
	{
