...
//------------------------------------------------------------------------------
// Sort Queue Function: Sorts the queue by priority level. 
void sortQueue(struct Queue* wQ)
{
	int priority = 0;
	...
	while(target != NULL)
	{
		if(target->aRec.priority < priority)
		{
			wQ->currentJob = target;
			priority = target->aRec.priority;
		}
