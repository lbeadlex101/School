...
//------------------------------------------------------------------------------
// Sort Queue Function: Sorts the queue by calculating shortest remaining time.
void sortQueue(struct Queue* wQ)
{
	int currentERT, currentST, targetERT, targetST, totalSRT, newSRT = 0;
	struct ListNode* target = NULL;
	
	// set target node to head of queue
	target = getNextPointer(wQ->head);
	
	// Calculate Shortest Remaining Time
	currentERT = wQ->currentJob->aRec.estRunTime;
	currentST = wQ->currentJob->aRec.serviceTime;
	totalSRT = currentERT - currentST;
	
	while(target != NULL)
	{
		targetERT = target->aRec.estRunTime;
		targetST = target->aRec.serviceTime;
		newSRT = targetERT - targetST;
