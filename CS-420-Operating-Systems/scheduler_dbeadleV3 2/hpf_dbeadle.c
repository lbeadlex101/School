...
//	CS 420
//	Scheduler 
//	Highest Priority First (HPF) (NOT CURRENTLY FUNCTIONAL)
//	This program implements the HPF scheduling algorithm. 
//------------------------------------------------------------------------------
#include <stdio.h>
#include <stdlib.h>
#include <stdbool.h>
...
int main() 
{
    ...
    {
    	printf("hpf_outFile.txt open for writng\n");
    }
...
//------------------------------------------------------------------------------
// Sort Queue Function: Sorts the queue by priority level. (INCOMPLETE)
// This function does not organize priority levels and prints debugging
// statements. 
void sortQueue(struct Queue* wQ)
{
	int priority = 0;
	...
	// set overall priority to current job priority level
	priority = wQ->currentJob->aRec.priority;
	
	if(wQ->head->aRec.priority < priority)
	{
		wQ->currentJob = target;
	}
	
	printf("Current Priority: %d \t", wQ->currentJob->aRec.priority);
...
void showOutput(struct Queue q, int CP)
{
	printf("%s \t %d \t %d \t %d \t %d \t\n", q.currentJob->aRec.jobName, 
			...
			q.currentJob->aRec.waitTime, CP, q.currentJob->aRec.priority);
}
//------------------------------------------------------------------------------
				...
				struct Queue qObj)
{
	...
	while (isListEmpty(listObj) == false || isQueueEmpty(qObj) == false )
	{
		...
		if (isQueueEmpty(qObj) == false)
		{
			...
			{
				// Organize by highest priority level
				sortQueue(&qObj);
				//printf("Queue Size: %d\n", qObj.queueSize);
