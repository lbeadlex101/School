...
//	CS 420
//	Scheduler 
//	Shortest Remaining Time (SRT) (NOT CURRENTLY FUNCTIONAL)
//	This program implements the SRT scheduling algorithm. 
//------------------------------------------------------------------------------
#include <stdio.h>
#include <stdlib.h>
#include <stdbool.h>
...
void sortQueue(struct Queue* wQ);
// Misc. Functions
...
int main() 
{
    ...
    {
    	printf("srt_outFile.txt open for writng\n");
    }
...
//------------------------------------------------------------------------------
// Sort Queue Function: Sorts the queue by shortest remaining time. (INCOMPLETE)
// This function does not organize srt levels and prints debugging
// statements. 
void sortQueue(struct Queue* wQ)
{
	int currentJobTime, shortestRemainingTime, ert, st = 0;
	struct ListNode* target;
	
	setNullPointer(target);
	
	// set target node to head of queue
	target = getNextPointer(wQ->head);
	shortestRemainingTime = wQ->currentJob->aRec.estRunTime;
	ert = wQ->currentJob->aRec.estRunTime;
	st = wQ->currentJob->aRec.serviceTime;

	currentJobTime = ert - st;

	if(shortestRemainingTime < currentJobTime)
	{
		wQ->currentJob = target;
	}
	
	printf("SRT: %d \t", shortestRemainingTime);
}
// MISC Functions
//------------------------------------------------------------------------------
...
void showOutput(struct Queue q, int CP)
{
	printf("%s \t %d \t %d \t %d \t %d \t\n", q.currentJob->aRec.jobName, 
			...
			q.currentJob->aRec.waitTime, CP, q.currentJob->aRec.estRunTime);
}
//------------------------------------------------------------------------------
